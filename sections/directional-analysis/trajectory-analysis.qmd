...
Currently these functions allow for the import of pre-calculated back trajectories at several pre-defined locations and some trajectory plotting functions. In time, all of these functions will be developed to allow more sophisticated analyses to be undertaken.

The `importTraj()` function imports pre-calculated back trajectories using the Hysplit trajectory model [Hybrid Single Particle Lagrangian Integrated Trajectory Model](https://ready.arl.noaa.gov/HYSPLIT.php) [@stein2015]. Trajectories are run at 3-hour intervals and stored in yearly files (see below). The trajectories are started at ground-level (10m) and propagated backwards in time. The data are stored on web-servers at WSP similar to that for [importUKAQ](#sec-importUKAQ), which makes it very easy to import pre-processed trajectory data for a range of locations and years. [^trajectory-analysis-1]
...
#| fig-width: 6
#| fig-height: 6
selectByDate(traj, start = "15/4/2010", end = "21/4/2010") |>
  trajPlot(
...
#| fig-width: 8
#| fig-height: 5
filter(traj, lat > 30, lat < 70, lon > -50, lon < 20) |>
  trajLevel(
    method = "hexbin",
    cols = "turbo",
    xbin = 40
...
```{r}
#| label: allocateTraj
x0 <- traj$lon[traj$hour.inc == 0][1]
y0 <- traj$lat[traj$hour.inc == 0][1]

alloc <- traj |>
  mutate(
    angle = (atan2(lon - x0, lat - y0) * 180 / pi) %% 360,
    sector = cut(angle, breaks = seq(22.5, 382.5, 45),
                 labels = c("NE", "E", "SE", "S", "SW", "W", "NW", "N")) |>
      as.character() |>
      coalesce("N")
  ) |>
  count(date, sector) |>
  slice_max(n, by = date, with_ties = FALSE) |>
  mutate(sector = if_else(n > 50, sector, "unallocated"))
...
```{r}
#| eval: false
trajLevel(clust$data$traj, type = "cluster", cols = "increment", border = NA)
```
...
kc1 <- inner_join(kc1, filter(clust$data$traj, hour.inc == 0), by = "date")
```
...
## Reprojecting trajectories

Both of `trajPlot()` and `trajLevel()` contain the `crs` argument, which is passed to `ggplot2::coord_sf()` via `sf::st_crs()`. Both of these latter two functions document `crs` extensively but, in short, it is a way to define a coordinate reference system to reproject the plotted data.

By default, the `crs` is [4326](https://epsg.io/4326), the "World Geodetic System" (WGS84), which is associated with the globally ubiquitous "latitude and longitude" coordinate system. However, WGS84/ESPG:4326 is a geographic coordinate system — it describes positions on a sphere — rather than a projected one that flattens that sphere onto a 2D plane. This means shapes and distances can appear distorted, particularly at higher latitudes or across wide spatial extents. Choosing a projected CRS better suited to your region of interest can produce a more accurate and visually appealing map.

@fig-crs-latlng, @fig-crs-bng and @fig-crs-lambert show the same back trajectory data plotted with different CRSs. The first plot uses the default WGS84/ESPG:4326, which is a geographic coordinate system. The second plot uses the British National Grid (BNG) CRS, which is a projected coordinate system commonly used in the UK. The third plot uses the Lambert Azimuthal Equal Area (LAEA) CRS, which is a projected coordinate system often used for European data.

::: {.callout-warning}
`crs` can take a wide variety of arguments, including custom projections written in the PROJ string form. It's up to you to pick a suitable `crs` for your data, as poorly chosen coordinate reference systems may obscure the data or cause visual artefacts. For example, crs = 27700 (the British National Grid) is well-suited for trajectories surrounding the UK, but will produce increasing distortion for trajectories that extend far beyond it.
:::

```{r}
#| label: fig-crs-latlng
#| fig-cap: Trajectories plotted in the default WGS84/ESPG:4326 coordinate reference system (CRS) using `trajLevel()`.
trajLevel(traj, crs = 4326)
```

```{r}
#| label: fig-crs-bng
#| fig-cap: Trajectories plotted in the British National Grid (BNG) coordinate reference system (CRS) using `trajLevel()`. 
trajLevel(traj, crs = 27700)
```

```{r}
#| label: fig-crs-lambert
#| fig-cap: Trajectories plotted in the Lambert Azimuthal Equal Area (LAEA) coordinate reference system (CRS) using `trajLevel()`. This projection is often used for European data and is a good choice for this dataset.
trajLevel(traj, crs = 3035)
```
...
Plot an interactive map. Try clicking on the individual points to see information like the air mass height, pressure and PM~10~ concentration.
