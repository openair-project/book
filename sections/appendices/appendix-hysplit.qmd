...
As discussed in @sec-trajPlot, [openair]{.pkg} can import pre-calculated trajectory data for specified locations for whole years run at a 3-hour interval. The data are stored on a WSP webserver to make it easy to import 96-hour back trajectory data. Several users have requested how they can run HYSPLIT themselves e.g. for different trajectory start heights or for many locations. It should be noted that Hysplit is a powerful and flexible model and to exploit its full potential, users should consider using Hysplit directly [@stein2015].
...
getMet <- function(
  year = 2013,
  month = 1,
  path_met = file.path("~", "TrajData"),
  skip_existing = TRUE
) {
  path_met <- normalizePath(path_met, mustWork = FALSE)
  if (!dir.exists(path_met)) {
    dir.create(path_met, recursive = TRUE, showWarnings = FALSE)
  }

  base_url <- "https://noaa-oar-arl-hysplit-pds.s3.amazonaws.com/reanalysis"
  ...
  for (i in seq_along(year)) {
    for (j in seq_along(month)) {
      ...
      filename <- sprintf("RP%d%02d.gbl", yr, mo)
      destfile <- file.path(path_met, filename)

      if (skip_existing && file.exists(destfile)) {
        message("  Skipping (already exists): ", filename)
        next
      }

      # New S3 structure: reanalysis/{year}/{filename}
      url <- paste0(base_url, "/", yr, "/", filename)
