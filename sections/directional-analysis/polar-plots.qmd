...
polarPlot(mydata, pollutant = "ratio", title = "so2/nox ratio")
```
...
met <- import_isd_hourly(code = "036580-99999", year = 2013)
...
#| fig-height: 4.5
#| fig-keep: 'last'
clust_out <- polarCluster(
  selectByDate(port_talbot, year = 2012),
  after = selectByDate(port_talbot, year = 2019),
  pollutant = "so2",
  n.clusters = 5
)
...
A more useful output is to track the concentrations before and after based on @fig-polarClust-diff, which can be achived using the code below. The results show the greatest reduction in SO~2~ concentrations is in Cluster 3, which is the cluster most strongly associated with the change in SO~2~ concentrations. This is a good example of how clustering differences can be used to identify the features that have changed the most between two time periods.

```{r}
#| label: trackClustDiff
bind_rows(
  before = clust_out$data,
  after = clust_out$after,
  .id = "period"
) |>
  group_by(cluster, period) |>
  summarise(mean_so2 = mean(so2, na.rm = TRUE), .groups = "drop") |>
  tidyr::pivot_wider(names_from = period, values_from = mean_so2) |>
...
[openairmaps]{.pkg} isn't limited to just `polarMap()` --- almost any kind of "directional analysis" plot can be placed on a map. There are many similar functions which correspond to other [openair]{.pkg} plotting functions; `annulusMap()`, `freqMap()`, `percentileMap()`, `polarMap()`, `pollroseMap()`, `windroseMap()`, and `diffMap()`.
