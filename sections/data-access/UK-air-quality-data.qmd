...
To import data, you can use the `importUKAQ()` function. Some
examples are below.
...
## managed' sites may have overlapping site codes w/ nationally managed sites -
## e.g., "AD1")
...
kc1_zagglom <- importUKAQ(site = "kc1", year = 2018, meta = TRUE, meta_columns = c("zone", "agglomeration"))
glimpse(kc1_zagglom)
```
...
-   **"daily"** Daily means returned and specific site(s) must be provided. Note that in the case of PM~10~ and PM~2.5~ daily measurements can be available those derived from hourly measurements (using instruments such as TEOM, BAM and FIDAS) and our daily gravimetric measurements such as from a Partisol. In the returned data the gravimetric daily measurements are shown as `gr_pm10` and `gr_pm2.5`, respectively.
