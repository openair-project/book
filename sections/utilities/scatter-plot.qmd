...
-  It deals with wind speed and wind direction data correctly. For example `type = "wd"` will produce a 8 panel plot by wind sector. It can also add 'wind flow' arrows to scatter plots.
...
-   It can handle date/time x-axis formats to provide an alternative way of showing time series, which again can be colour-coded by a third variable. It is also easy to take account of different time-averaging periods e.g. hourly, daily, weekly etc. using the `timeAverage` function and the argument `avg.time`.
...
#| out-width: 85%
scatterPlot(
  selectByDate(mydata, year = 2003, month = 8),
  x = "date",
  y = "so2",
  z = "wd",
  plot.type = "b"
...
#| out-width: 75%
scatterPlot(
  selectByDate(mydata, start = "1/6/2001", end = "5/6/2001"),
  x = "date",
  y = "no2",
  z = "o3",
  size = 3,
  cols = "increment",
  windflow = windflowOpts(colour = "grey50", linewidth = 0.8),
  ylab = "no2 (ppb)"
)
```
