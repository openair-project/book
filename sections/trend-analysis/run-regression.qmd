...
met <- import_isd_hourly(year = 2022)
...
out_filter <- filter(
  out,
  r_squared > 0.95,
  slope > quantile(slope, probs = 0.01, na.rm = TRUE),
  slope < quantile(slope, probs = 0.99, na.rm = TRUE),
  delta_nox > 100
)
```
...
aq_select <-
  map(seq_len(nrow(out_filter)), function(i) {
    filter(
      aq,
...
ggplot(
  ...
  labs(
    y = quickText("pm2.5 (ug/m3)"),
    x = quickText("nox (ug/m3)"),
    colour = quickText("Background\nPM2.5")
  )
```
