...
The `...$plot` component is a list of individual panels, and the `...$data` component is a list of dataframes. Note that `mid`, `min` and `max` are the mean and upper/lower confidence intervals when `statistic = "mean"` and the median and upper/lower quantiles when `statistic = "median"`. Each part of the list is named after its respective panel.

```{r}
#| label: timevarOutput
# create plot
myOutput <- timeVariation(mydata, pollutant = "so2", plot = FALSE)

# see what's available
myOutput$data$subsets

# get an element of the TV plot
hour_weekday_df <- myOutput$data$hour.weekday

# print
head(hour_weekday_df)
```
It is also possible to plot any of the individual plots that make up `timeVariation()`.

```{r}
#| eval: false
plot(myOutput, subset = "hour")
# ...or plot(myOutput$plot$hour)
```

Note if you are only interested in one panel, the `variationPlot()` function may be more suitable. It has many of the same arguments as `timeVariation()`, though has the added advantage of being able to take two `type` arguments.

```{r}
variationPlot(mydata, pollutant = "so2", x = "hour", type = c("season", "weekend"))
```
