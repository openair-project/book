...
the Internet. The first is hourly data for a site in London and the
second is hourly data for a site in Yorkshire. The data are then joined
...
# data from yorkshire
yorks_met <- readr::read_rds(
  "https://github.com/openair-project/book/raw/refs/heads/main/assets/data/yorkshire_met.rds"
)
...
set (Yorkshire) tends to be positively or negatively biased with respect
to the reference data set (London). It shows for example:

-  Wind speeds at the Yorkshire site tend to be higher than at London for wind directions from the north-west and south-east, shown by the red colouring in @fig-windComp.

-  There are slightly slow wind speeds at the Yorkshire site whne the winds are from the WSW, shown by the blue colouring in @fig-windComp.

-  There is a slight bias in the wind direction of -4.3 degrees at the Yorkshire site — shown by both the arrow on the plot and the text.

```{r}
#| label: fig-windComp
#| fig-cap: Wind rose showing the difference between two meteorological data sets. The first (London) is used to compare against the second (Yorkshire). The colours are used to show whether data tend to be positively or negatively biased with respect to the reference data set. The arrow shows the mean bias in wind direction and the text shows the value of that bias.
#| fig-width: 4.5
#| fig-height: 5.5
#| out-width: 75%
windRose(
