...
Let's put two plots side by side (2 columns, 1 row). First it is necessary to assign the plots to a variable:

```{r}
#| include: false
a <- windRose(mydata, key.position = "bottom")
b <- polarPlot(mydata, key.position = "bottom")
```

Now let's load [patchwork]{.pkg} and simply add the two plots together.
