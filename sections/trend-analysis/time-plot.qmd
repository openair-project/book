...
With packages such as [ggplot2]{.pkg} it is very easy to plot time series. However, there are a few enhancements in `timePlot()` that are useful for the analysis of air quality data including:

-  The ability to flexibly time average the data in several ways e.g. to plot daily or monthly means from hourly data, or hourly means from 15-minute data.
-  The ability to plot multiple sites and pollutants in separate panels or grouped together in one panel using the `type` and `group` arguments.
-  The ability to normalise data to a common scale of 100 at the start of time series, which is useful for comparing time series on different scales e.g. emissions and ambient measurements.
-  The ability to plot wind vectors on time series plots, which is useful for understanding the relationship between pollution and wind speed/direction.
-  Automatic formatting of common pollutant names and units.
...
timePlot(
  selectByDate(mydata, year = 2003, month = "aug"),
...
In @fig-importAQdata the y-scale is the same by default for all panels. However, the `scales` argument can be used to set the y-scales to be set individually for each panel by setting `scales = "free_y"`. Other options include `scales = "free_x"` for individual x-axis scales and `scales = "free"` for this to apply to both the x- and y-axes.

Using the code above it is also possible to include several species. But what if we wanted to plot NO~x~ concentrations across all sites in one panel? An example of how to do this is shown below. In this case the `group` argument is used to display the concentrations from multiple sites in one panel.
