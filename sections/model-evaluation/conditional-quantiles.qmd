...
#| label: loadCMAQ
load(url(
  "https://github.com/openair-project/book/raw/refs/heads/main/assets/data/CMAQozone.RData"
))
...
#| out-width: 75%
conditionalQuantile(
  filter(CMAQ.KCL, site == "Lullington.Heath"),
  obs = "o3",
  mod = "mod",
  type = "season"
)
...
A subset of the data for the North Kensington site can be imported as shown below. The file contains observed and modelled hourly values for O~3~, NO~x~, wind speed, wind direction, temperature and relative humidity.

```{r}
load(url(
  "https://github.com/openair-project/book/raw/refs/heads/main/assets/data/condDat.RData"
))
...
conditionalEval(
  ...
  statistic = "NMB",
  var.names = c("temperature", "wind speed")
)
```
...
## join with orginal model eval data
out <- inner_join(condDat, cl$data$traj, by = "date")
